      ...
      <ul className="space-y-2">
        {filteredUsers.map((user) => (
          <li key={user._id} className="border p-3 rounded shadow flex flex-col">
            ...
            {user.active && user.approvedBy && (
              <div className="text-green-600">Approved by {user.approvedBy} on {formatDate(user.approvedAt)}</div>
            )}
            {!user.active && user.approvedBy && (
              <div className="text-gray-500 italic">Previously approved by {user.approvedBy} on {formatDate(user.approvedAt)}</div>
            )}
            {user.deactivatedBy && (
              <div className="text-red-600">Deactivated by {user.deactivatedBy} on {formatDate(user.deactivatedAt)}</div>
            )}
            {user.reactivatedBy && (
              <div className="text-blue-600">Reactivated by {user.reactivatedBy} on {formatDate(user.reactivatedAt)}</div>
            )}
